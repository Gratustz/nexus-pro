...
function SignalBadge({ signal }: { signal: string }) {
  ...
  return (
    <span className={`border text-xs font-bold px-3 py-1 rounded-full ${colors[signal] || 'bg-gray-800 text-gray-400 border-gray-700'}`}>
      {signal}
    </span>
  )
}
...
function CryptoCard({ symbol }: { symbol: string }) {
  ...
  const price = symbol === 'BTCUSDT' ? '67,234.50' :
    symbol === 'ETHUSDT' ? '3,456.20' :
    symbol === 'SOLUSDT' ? '145.30' :
    symbol === 'BNBUSDT' ? '412.80' : '0.5823'
...
function SportsCard({ match }: { match: string }) {
  ...
  return (
    <div className="bg-gray-900 border border-gray-800 rounded-xl p-5 hover:border-gray-700 transition">
      ...
      <p className="text-gray-500 text-xs">Top score: 2-0 (18.3%) · 2-1 (14.2%)</p>
    </div>
  )
}
...
export default function Dashboard() {
  ...
  const matches = ['Arsenal vs Chelsea', 'Man City vs Liverpool', 'Spurs vs Man Utd']
      ...
      <div className="max-w-7xl mx-auto px-4 py-8">
        ...
        <div className="flex items-center justify-between mb-8">
          <div>
            ...
            <p className="text-gray-400 mt-1">Live signals across all markets</p>
          </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
          {[
            { label: 'Crypto Signals', value: '30', sub: '5 symbols × 6 timeframes' },
        ...
        <div className="bg-blue-500/10 border border-blue-500/20 rounded-xl p-4 mb-6 flex items-center justify-between">
          <div>
            <p className="text-blue-400 font-medium text-sm">
              You are on the {user?.plan?.toUpperCase()} plan
            </p>
        ...
        {activeTab === 'crypto' && (
          <div>
            <h2 className="text-lg font-bold text-white mb-4">Crypto Signals</h2>
        ...
        {activeTab === 'sports' && (
          <div>
            <h2 className="text-lg font-bold text-white mb-4">Sports Predictions</h2>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {matches.map(match => (
